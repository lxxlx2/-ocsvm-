...
def test(svm, test_dir, test_result_dir, nu, gamma):
    ...
    for file_name in os.listdir(test_dir):  # 遍历test_dir
        ...
        # # features = rgb
        # features = np.concatenate([features, rgb], axis=0)
...
def log_softmax(x):
    x = x - np.max(x)  # 减去max值,为了数值稳定性
    exp_x = np.exp(x)
    return x - np.log(exp_x.sum())
